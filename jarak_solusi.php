...
$i3=0;
$maxarray=array();
$a2=mysqli_query($connect, "select * from kriteria");
echo "<tr>";
while($da2=mysqli_fetch_assoc($a2)){
		...
		//ambil nilai
			...
			$n2=mysqli_query($connect,"select * from nilai_matrik where id_kriteria='$idalt2'");
		$jarakp2=0;
		$c2=0;
		$ymax2=array();
		...
		while($dn2=mysqli_fetch_assoc($n2)){
			...
			$nilai_kuadrat2=0;
			$k2=mysqli_query($connect, "select * from nilai_matrik where id_kriteria='$idk2' ");
			while($dkuadrat2=mysqli_fetch_assoc($k2)){
				$nilai_kuadrat2=$nilai_kuadrat2+($dkuadrat2['nilai']*$dkuadrat2['nilai']);
			}

			//hitung jml alternatif
			$jml_alternatif2=mysqli_query($connect, "select * from alternatif");
			...
			$k22=mysqli_query($connect, "select * from nilai_matrik where id_kriteria='$idk2' ");
			...
			//nilai bobot input
			$b2=mysqli_query($connect,"select * from kriteria where id_kriteria='$idk2'");
...
$a=mysqli_query($connect, "select * from alternatif");
echo "<tr>";
while($da=mysqli_fetch_assoc($a)){
		...
		//ambil nilai			
			$n=mysqli_query($connect, "select * from nilai_matrik where id_alternatif='$idalt'");
		$jarakp=0;
		$c=0;
		$ymax=array();
		$arraymaks=array();
		while($dn=mysqli_fetch_assoc($n)){
			...
			//nilai kuadrat			
			$nilai_kuadrat=0;
			$k=mysqli_query($connect,"select * from nilai_matrik where id_kriteria='$idk' ");
			while($dkuadrat=mysqli_fetch_assoc($k)){
				$nilai_kuadrat=$nilai_kuadrat+($dkuadrat['nilai']*$dkuadrat['nilai']);
			}

			//hitung jml alternatif
			$jml_alternatif=mysqli_query($connect,"select * from alternatif");
			...
			$k2=mysqli_query($connect,"select * from nilai_matrik where id_kriteria='$idk' ");
			...
			//nilai bobot input
			$b2=mysqli_query($connect,"select * from kriteria where id_kriteria='$idk'");
